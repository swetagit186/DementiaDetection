...
#                 if mid_slice.ndim == 2:
#                     normalized = cv2.normalize(mid_slice, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
#                     # Convert numpy array directly to a tensor
#                     pil_image = Image.fromarray(normalized)
#                     st.image(pil_image, caption="Middle Slice (Grayscale)", width=300)
#                 elif mid_slice.ndim == 3 and mid_slice.shape[2] == 3:
#                     normalized = cv2.normalize(mid_slice, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
#                     # Convert numpy array directly to a tensor
#                     pil_image = Image.fromarray(normalized)
#                     st.image(pil_image, caption="Middle Slice (RGB)", width=300)
#                 else:
#                     st.warning("⚠️ Unsupported image shape for display.")
#             else:
#                 st.error("❌ Could not convert middle slice to an image.")
...
#                 transforms.Resize((128, 128)),
#                 transforms.Grayscale(num_output_channels=1),
#                 transforms.ToTensor()  # Directly apply ToTensor to the PIL image
#             ])
#
